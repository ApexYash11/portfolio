...
export const DATA = {
  ...
  work: [
    ...
    },
    {
      ...
      description:
        "Session Kill State Isolation (#3084) • Fixed kill button 'Killing…' state leaking across session switches by keying TopbarKillButton with session.id. • Added a per-mutate success callback to prevent late kill completion from navigating to the wrong session. • Updated regression tests to assert deferred navigation is correctly skipped. Tmux IsAlive Agent Exit Detection (#2831) • Fixed IsAlive() to correctly detect when an agent has exited even if the tmux pane remains open. • Added a pane-scoped @ao_agent_exited marker set before the keep-alive shell starts and queried in IsAlive. • Decoupled Create from IsAlive to avoid false session destruction on fast agents. • Quiet show-options -qv lookup preserves three states: unset=alive, 1=dead, command failures=probe errors. Reviewer Binary Preflight Check (#2092, #2767) • Added a preflight validation step in the review trigger that checks the reviewer binary is on PATH before creating review runs. • Built a ReviewerPreflighter interface supporting Codex and Claude Code adapters. • Prevents silent failed/0-findings outcomes when the binary is missing. Prompt Byte Metrics (#2839) • Surfaced assembled prompt and system prompt byte lengths during agent spawn. • Threaded promptBytes and systemPromptBytes through the spawn response and CLI output. • Regenerated the OpenAPI spec and added test coverage for the new metrics.",
    },
  ],
  education: [
    {
